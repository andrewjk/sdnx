...
@Suite("Parse tests") struct ParseTests {
    @Test func basicTest() throws {
        ...
        let result: OrderedDictionary<String, Any>
        switch parseResult {
        case .success(let success):
            result = success.data
        case .failure(let failure):
            #expect(Bool(false), "Parse failed with errors: \(failure.errors.map { $0.message }.joined(separator: ", "))")
            return
        }
        ...
        // based on the indentation of second line
        ...
        let spacedParseResult = parse(spacedInput)
        let spacedResult: OrderedDictionary<String, Any>
        switch spacedParseResult {
        case .success(let success):
            spacedResult = success.data
        case .failure(let failure):
            #expect(Bool(false), "Parse failed with errors: \(failure.errors.map { $0.message }.joined(separator: ", "))")
            return
        }
        ...
        let unspacedParseResult = parse(unspacedInput)
        let unspacedResult: OrderedDictionary<String, Any>
        switch unspacedParseResult {
        case .success(let success):
            unspacedResult = success.data
        case .failure(let failure):
            #expect(Bool(false), "Parse failed with errors: \(failure.errors.map { $0.message }.joined(separator: ", "))")
            return
        }
