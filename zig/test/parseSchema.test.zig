...
test "parse schema: basic test" {
    ...
    {
        var result = try parseSchema.parseSchema(allocator, input);
        defer result.deinit();
        try std.testing.expect(result.schema.count() == 8);
    }

    // Test spaced input
    const spaced_input = try space_unspace.space(allocator, input);
    defer allocator.free(spaced_input);
    {
        var result = try parseSchema.parseSchema(allocator, spaced_input);
        defer result.deinit();
        try std.testing.expect(result.schema.count() == 8);
    }

    // Test unspaced input
    const unspaced_input = try space_unspace.unspace(allocator, input);
    defer allocator.free(unspaced_input);
    {
        const fixed_unspaced = try space_unspace.applyUnspaceReplacements(allocator, unspaced_input);
        defer allocator.free(fixed_unspaced);
        var result = try parseSchema.parseSchema(allocator, fixed_unspaced);
        defer result.deinit();
        try std.testing.expect(result.schema.count() == 8);
    }
}
...
test "parse schema: simple type" {
    ...
    defer allocator.free(unspaced_input);
    {
        var result = try parseSchema.parseSchema(allocator, unspaced_input);
        defer result.deinit();
        try std.testing.expect(result.schema.count() == 1);
        const name_entry = result.schema.get("name");
        try std.testing.expect(name_entry != null);
...
test "parse schema: mix macro" {
    ...
    defer allocator.free(unspaced_input);
    {
        const fixed_input = try space_unspace.applyUnspaceReplacements(allocator, unspaced_input);
        defer allocator.free(fixed_input);
        var result = try parseSchema.parseSchema(allocator, fixed_input);
        defer result.deinit();
        try std.testing.expect(result.schema.count() == 3);
        const name_entry = result.schema.get("name");
        try std.testing.expect(name_entry != null);
